define(function(){
	 ...
	 * usage
	 *
	 * myEmitter.on( ALL_EVENT, function( eventName, argsArray ){
	 * 	//do stuff
	 * });
	 *
	 * @type {String}
	...
	return function EventEmitter()
	{
		...
		this.emit = function( sEvent )
		{
			...
			for( i = 0; i < this._mSubscriptions[ ALL_EVENT ].length; i++ )
			{
				ctx = this._mSubscriptions[ ALL_EVENT ][ i ].ctx || {};
				this._mSubscriptions[ ALL_EVENT ][ i ].fn.apply( ctx, args );
			}
		};
